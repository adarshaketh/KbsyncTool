...
static inline mach_msg_return_t LMMachMsg(LMConnection *connection, mach_msg_header_t *msg, mach_msg_option_t option,
    ...
    for (;;) {
        ...
        if (connection->serverPort == MACH_PORT_NULL) {
            ...
            if ((kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_5_0) &&
                (kCFCoreFoundationVersionNumber < 800.0)) {
                int sandbox_result =
                    sandbox_check(getpid(), "mach-lookup",
                                  (enum sandbox_filter_type)(SANDBOX_FILTER_LOCAL_NAME | SANDBOX_CHECK_NO_REPORT),
...
static inline kern_return_t LMConnectionSendOneWay(LMConnectionRef connection, SInt32 messageId, const void *data,
                                                   uint32_t length) {
    // Send message
    ...
    return LMMachMsg(connection, &message->head, MACH_SEND_MSG, size, 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE,
                     MACH_PORT_NULL);
}
...
static inline kern_return_t LMConnectionSendTwoWay(LMConnectionRef connection, SInt32 messageId, const void *data,
                                                   uint32_t length, LMResponseBuffer *responseBuffer) {
    // Create a reply port
    mach_port_t selfTask = mach_task_self();
    ...
    LMMessageAssignData(message, data, length);
    err = LMMachMsg(connection, &message->head, MACH_SEND_MSG | MACH_RCV_MSG | _LIGHTMESSAGING_TIMEOUT_FLAGS, size,
                    sizeof(LMResponseBuffer), replyPort, LIGHTMESSAGING_TIMEOUT, MACH_PORT_NULL);
    if (err)
        responseBuffer->message.body.msgh_descriptor_count = 0;
    // Cleanup
...
static inline void LMResponseBufferFree(LMResponseBuffer *responseBuffer) {
    if (responseBuffer->message.body.msgh_descriptor_count != 0 &&
        responseBuffer->message.data.out_of_line.descriptor.type == MACH_MSG_OOL_DESCRIPTOR) {
        vm_deallocate(mach_task_self(), (vm_address_t)responseBuffer->message.data.out_of_line.descriptor.address,
                      responseBuffer->message.data.out_of_line.descriptor.size);
        responseBuffer->message.body.msgh_descriptor_count = 0;
    }
}

#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
static inline kern_return_t LMStartServiceWithUserInfo(name_t serverName, CFRunLoopRef runLoop,
                                                       CFMachPortCallBack callback, void *userInfo) {
...
static inline kern_return_t LMCheckInService(name_t serverName, CFRunLoopRef runLoop, CFMachPortCallBack callback,
                                             void *userInfo) {
    // TODO: Figure out what the real interface is, implement service stopping, handle failures correctly
    mach_port_t bootstrap = MACH_PORT_NULL;
    task_get_bootstrap_port(mach_task_self(), &bootstrap);
...
static inline bool LMDataWithSizeIsValidMessage(const void *data, CFIndex size) {
    ...
    if ((size_t)size < sizeof(mach_msg_header_t) + sizeof(mach_msg_body_t) + sizeof(uint32_t))
        return false;
    if ((size_t)size <
        sizeof(mach_msg_header_t) + sizeof(mach_msg_body_t) + sizeof(uint32_t) + message->data.in_line.length)
        return false;
    return true;
}
...
static inline kern_return_t LMSendReply(mach_port_t replyPort, const void *data, uint32_t length) {
    ...
    // Send message
    kern_return_t err =
        mach_msg(&response->head, MACH_SEND_MSG, size, 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
    if (err) {
...
static inline NSData *LMDataForPropertyList(id propertyList) {
    ...
    return [NSPropertyListSerialization dataFromPropertyList:propertyList
                                                      format:NSPropertyListBinaryFormat_v1_0
...
static inline kern_return_t LMConnectionSendTwoWayData(LMConnectionRef connection, SInt32 messageId, CFDataRef data,
...
static inline int32_t LMResponseConsumeInteger(LMResponseBuffer *buffer) {
    int32_t result = LMMessageGetDataLength(&buffer->message) == sizeof(int32_t)
                         ? *(int32_t *)buffer->message.data.in_line.bytes
                         : 0;
...
static inline kern_return_t LMConnectionSendTwoWayPropertyList(LMConnectionRef connection, SInt32 messageId,
                                                               id propertyList, LMResponseBuffer *buffer) {
    return LMConnectionSendTwoWayData(
        connection, messageId, propertyList ? LMBridgedCast_(CFDataRef, LMDataForPropertyList(propertyList)) : NULL,
        buffer);
}
...
static inline id LMResponseConsumePropertyList(LMResponseBuffer *buffer) {
    ...
    if (length) {
        CFDataRef data = CFDataCreateWithBytesNoCopy(
            kCFAllocatorDefault, (const UInt8 *)LMMessageGetData(&buffer->message), length, kCFAllocatorNull);
...
static inline UIImage *LMResponseConsumeImage(LMResponseBuffer *buffer) {
    if (buffer->message.body.msgh_descriptor_count != 0 &&
        buffer->message.data.out_of_line.descriptor.type == MACH_MSG_OOL_DESCRIPTOR) {
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(
            NULL, bytes, buffer->message.data.out_of_line.descriptor.size, LMCGDataProviderReleaseCallback);
        if (provider) {
            CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
            CGImageRef cgImage = CGImageCreate(header->width, header->height, header->bitsPerComponent,
                                               header->bitsPerPixel, header->bytesPerRow, colorSpace,
                                               header->bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
static inline kern_return_t LMSendImageReply(mach_port_t replyPort, UIImage *image) {
        ...
        if (cgImage) {
            ...
            if (&CGAccessSessionCreate != NULL) {
                ...
                if (accessSession) {
                    ...
                    if (pointer) {
                        LMMessageAssignOutOfLine(&buffer.response, pointer,
                                                 buffer.imageHeader.bytesPerRow * buffer.imageHeader.height);
                        hasLoadedData = true;
                    }
                }
            }
    ...
    // Send message
    kern_return_t err = mach_msg(&buffer.response.head, MACH_SEND_MSG, sizeof(buffer), 0, MACH_PORT_NULL,
